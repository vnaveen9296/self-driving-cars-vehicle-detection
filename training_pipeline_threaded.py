...
####################
# Thread function (target function) used by search_windows() function
def threaded_fn_for_search_windows(img, split_windows, result_windows, clf, scaler, color_space, spatial_size=(32,32), hist_bins=32, hist_range=(0,256), orient=9,
...
# and searches for cars within those windows
def search_windows(img, windows, clf, scaler, color_space, spatial_size=(32,32), hist_bins=32, hist_range=(0,256), orient=9,
    ...
    num_threads = 6
    threads = []
    # list of windows that need to be searched are split into multiple chunks where each chunk is handled by a separate thread
    split_windows = []
    # result windows is a list of list. Each thread is going to populate one of these lists
    result_windows = [[], [], [], [], [], []]
    batch_sz = math.ceil(len(windows)/num_threads)
    for start in range(0, len(windows), batch_sz):
        end = start + batch_sz
        split_windows.append(windows[start:end])

    for j in range(num_threads):
        #print('args: ', hex(id(split_windows[j])))
        args = (img, split_windows[j], result_windows[j], clf, scaler, color_space, spatial_size, hist_bins, hist_range, orient,
                        pix_per_cell, cell_per_block, hog_channel, use_spatial_bin, use_color_hist, use_hog_features)
        t = threading.Thread(target=threaded_fn_for_search_windows, args=args)
        threads.append(t)
        t.start()
    ...
    # create a single list of positive windows
    for w in result_windows:
        positive_windows.extend(w)
    end_time = time.time()
    #print('Time took for search_windows(): ', round(end_time-start_time, 2), 'seconds and t = {}, t2 = {}'.format(start_time, end_time))
    #ipdb.set_trace()
